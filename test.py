...
def _ensure_inference_defaults(args, config_file, config_keys=None):
    ...
    _set_if_not_present(args, "target_cache_batch_size", args.test_batch_size)
    _set_if_not_present(args, "target_query_batch_size", args.test_batch_size)
    ...
    _set_if_not_present(args, "loss_names", "sdm+id+mlm")
    _set_if_not_present(args, "temperature", 0.02)
    _set_if_not_present(args, "tau", 0.015)
...
def _build_parser():
    parser = argparse.ArgumentParser(
        description="IRRA checkpoint transfer evaluation",
    ...
    parser.add_argument("--enrichment_space", choices=("global", "grab", "retrieval"), default=None)
    parser.add_argument("--top_m", type=int, default=None)
    parser.add_argument(
        "--topm_rank_space",
        choices=("host_global", "retrieval", "hybrid_global_grab", "hybrid_global_retrieval"),
        default=None,
    )
    ...
    parser.add_argument("--target_cache_batch_size", type=int, default=None)
    parser.add_argument("--target_query_batch_size", type=int, default=None)
    _add_boolean_override(
        parser,
        "strict_target_checkpoint",
...
def main():
    ...
    logger = setup_logger("IRRA", save_dir=eval_dir, if_train=False)
